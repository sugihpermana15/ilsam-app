(function ($) {
    ...
    // Ensure GSAP plugins are registered (needed for text typing effect).
    if (window.gsap) {
        if (window.ScrollTrigger && gsap.registerPlugin) {
            gsap.registerPlugin(ScrollTrigger);
        }
        if (window.TextPlugin && gsap.registerPlugin) {
   ...
   Data Css js
   ========================================*/
    (function initDataBackgroundLazy() {
        const $els = $("[data-background]");
        if (!$els.length) return;

        const applyBg = (el) => {
            const $el = $(el);
            if ($el.attr("data-bg-loaded") === "1") return;
            const url = $el.attr("data-background");
            if (!url) return;
            $el.css("background-image", "url(" + url + ")");
            $el.attr("data-bg-loaded", "1");
        };
    ...
    // prelaoder
    // Guard GSAP targets to avoid console warnings on pages without the SVG.
    let span = $(".letter"),
        tlSmell = new TimelineMax({ repeat: -1 });

    const $smell = $("svg .smell");
    if ($smell.length) {
    ...
    class GSAPAnimation {
        ...
        static upDownScrollTriggerAnimation(titleClass) {
            ...
            Title.forEach((TitleSingle) => {
                const TitleSingleTarget =
                    TitleSingle.getAttribute("data-target");
    ...
    class RRDEVS {
        ...
        async LoadedAfterProgress(type) {
            ...
            const closeEl = document.querySelector(".preloader-close");
            const hidePreloaderFast = () => {
                if (closeEl) closeEl.style.display = "none";
                if (preloaderEl) preloaderEl.style.display = "none";
            };

            // If GSAP (or related plugins) are not ready, just hide quickly.
            if (typeof window.gsap === "undefined" || !preloaderEl) {
                hidePreloaderFast();
                return true;
            }
            ...
            if ("click" === type) {
                ...
                let randomStop = Math.random() * (0.85 - 0.5) + 0.5;
                let tl = gsap.timeline();
                const preloaderTextElement =
                    document.querySelector(".preloader-text");
                const newText = preloaderTextElement?.dataset?.text;

                if (!preloaderTextElement || !newText) {
                    hidePreloaderFast();
                    return true;
                }

                // Start from empty so TextPlugin animates like typing.
                preloaderTextElement.textContent = "";
                ...
                // Small wait to let the timeline complete before heavy inits.
            ...
            /*GSAPAnimation*/
            if (typeof GSAPAnimation !== "undefined" && GSAPAnimation?.Init) {
                GSAPAnimation.Init();
            }
        }
    ...
    preloaders.each(function (index, item) {
        if ($(item).data("preloader") === "active") {
            if ($(item).data("loaded") === "progress") {
                ...
                const start = () => RRDevsInit.LoadedAfterProgress("load");
                ...
                // Safety fallback: don't keep preloader forever.
                setTimeout(start, 1500);
            } else if ($(item).data("loaded") === "doted") {
                $(".preloader-close").on("click", () =>
                    RRDEVS.LoadedAfterDoted(),
                );

                const start = () => RRDEVS.LoadedAfterDoted();
                if (document.readyState === "loading") {
                    document.addEventListener("DOMContentLoaded", start, {
                        once: true,
                    });
    ...
    window.addEventListener("resize", function () {
        if (window.gsap?.globalTimeline?.clear) {
    ...
    let clients_testimonial__slider = new Swiper(
        ".clients-testimonial__slider",
        {
            slidesPerView: 1,
            spaceBetween: 30,
        ...
        magnetize(e) {
            ...
            this.settings.target.each((index, element) => {
                let $this = $(element),
                    ...
                    attraction =
                        $this.data("magnetic-attraction") ||
                        this.settings.attraction,
                    distance =
                        $this.data("magnetic-distance") ||
                        this.settings.distance,
                    deltaX = Math.floor(centerX - mouseX) * -1 * attraction,
                    deltaY = Math.floor(centerY - mouseY) * -1 * attraction,
                    mouseDistance = this.distanceFromMouse(
                        $this,
                        mouseX,
                        mouseY,
                    ),
    ...
    new MagneticButton({
        ...
        onEnter: function (data) {
            gsap.to(data.target, {
                scale: data.target[0].dataset.magneticZoom,
            });
        },
        onExit: function (data) {
    ...
    let banner__slider = new Swiper(".banner__slider", {
        ...
        on: {
            slideChange: () => {
                document
                    .querySelector(".timer-swiperAfter")
    ...
    $(document).ready(function () {
        function sliderAnimations(elements) {
            ...
            elements.each(function () {
                ...
                var $animationType =
                    "ribuid-animation " + $this.data("animation");
                $this.css({
