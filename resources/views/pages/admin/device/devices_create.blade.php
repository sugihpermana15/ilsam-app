...
@section('title', 'Tambah Device | IGI')
@section('title-sub', 'Master Device')
@section('pagetitle', 'Tambah Device')
...
@section('content')
  <div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
      <h5 class="card-title mb-0">Tambah Device</h5>
    ...
    <div class="card-body">
      ...
      <form action="{{ route('admin.devices.store') }}" method="POST">
        @csrf
        @include('pages.admin.device._form', ['device' => null])

        <div class="mt-3 d-flex gap-2">
          <button type="submit" class="btn btn-primary">
            <i class="fas fa-save"></i> {{ __('common.save') }}
          </button>
          <a href="{{ route('admin.devices.index') }}" class="btn btn-secondary">{{ __('common.cancel') }}</a>
        </div>
      </form>
    </div>
  </div>
