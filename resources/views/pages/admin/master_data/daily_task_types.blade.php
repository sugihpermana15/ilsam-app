...
@section('title', 'Master Task Type | IGI')

@section('title-sub', 'Master Data Daily Task - Task Type')
@section('pagetitle', 'Master Task Type')
...
@section('content')
  @php
    $canCreate = \App\Support\MenuAccess::can(auth()->user(), 'daily_task_types', 'create');
    $canUpdate = \App\Support\MenuAccess::can(auth()->user(), 'daily_task_types', 'update');
  @endphp
  <div class="row">
    ...
    <div class="col-12">
      <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
          <h5 class="card-title mb-0">Master Task Type</h5>
          <button type="button" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#addModal" {{ $canCreate ? '' : 'disabled' }}>
                  ...
                  <td>
                    <div class="d-flex gap-2">
                      <button type="button" class="btn btn-sm btn-outline-primary js-edit" data-bs-toggle="modal" data-bs-target="#editModal" {{ $canUpdate ? '' : 'disabled' }}
                        data-update-url="{{ route('admin.daily_task_types.update', $r->id) }}"
                        data-id="{{ $r->id }}" data-name="{{ $r->name }}" data-sort="{{ $r->sort_order }}" data-active="{{ $r->is_active ? '1' : '0' }}">
                        <i class="fas fa-pen"></i>
                      </button>

                      <form action="{{ route('admin.daily_task_types.toggle', $r->id) }}" method="POST">
                        @csrf
  ...
  <div class="modal fade" id="addModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        <form method="POST" action="{{ route('admin.daily_task_types.store') }}">
          @csrf
          <input type="hidden" name="modal_context" value="create">
          <div class="modal-header">
            <h5 class="modal-title">Add Task Type</h5>
  ...
  <div class="modal fade" id="editModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        <form method="POST" action="{{ $editRow ? route('admin.daily_task_types.update', $editRow->id) : '#' }}" id="editForm">
          @csrf
          @method('PUT')
          ...
          <div class="modal-header">
            <h5 class="modal-title">Edit Task Type</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body">
