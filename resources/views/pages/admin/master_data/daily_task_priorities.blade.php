...
@section('title', 'Master Priority | IGI')

@section('title-sub', 'Master Data Daily Task - Priority')
@section('pagetitle', 'Master Priority')
...
@section('content')
  @php
    $canCreate = \App\Support\MenuAccess::can(auth()->user(), 'daily_task_priorities', 'create');
    $canUpdate = \App\Support\MenuAccess::can(auth()->user(), 'daily_task_priorities', 'update');
  @endphp
  <div class="row">
    ...
    <div class="col-12">
      <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
          <h5 class="card-title mb-0">Master Priority</h5>
                  ...
                  <td>
                    <div class="d-flex gap-2">
                      <button type="button" class="btn btn-sm btn-outline-primary js-edit" data-bs-toggle="modal" data-bs-target="#editModal" {{ $canUpdate ? '' : 'disabled' }}
                        data-update-url="{{ route('admin.daily_task_priorities.update', $r->id) }}"
                        data-id="{{ $r->id }}" data-name="{{ $r->name }}" data-sort="{{ $r->sort_order }}" data-active="{{ $r->is_active ? '1' : '0' }}">
                        <i class="fas fa-pen"></i>
                      </button>

                      <form action="{{ route('admin.daily_task_priorities.toggle', $r->id) }}" method="POST">
                        @csrf
  ...
  <div class="modal fade" id="addModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        <form method="POST" action="{{ route('admin.daily_task_priorities.store') }}">
          @csrf
  ...
  @php
    $editRow = old('modal_context') === 'edit'
      ? $priorities->firstWhere('id', (int) old('id'))
      : null;
  @endphp
  ...
  <div class="modal fade" id="editModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        <form method="POST" action="{{ $editRow ? route('admin.daily_task_priorities.update', $editRow->id) : '#' }}" id="editForm">
          @csrf
          @method('PUT')
