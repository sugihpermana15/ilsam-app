...
@section('title', __('master_data.account_types.page_title') . ' | IGI')

@section('title-sub', __('master_data.account_types.subtitle'))
@section('pagetitle', __('master_data.account_types.pagetitle'))
...
@section('content')
  @php
    $canCreate = \App\Support\MenuAccess::can(auth()->user(), 'account_types', 'create');
    $canUpdate = \App\Support\MenuAccess::can(auth()->user(), 'account_types', 'update');
  @endphp
    ...
    <div class="col-12">
      <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
          <h5 class="card-title mb-0">{{ __('master_data.account_types.card_title') }}</h5>
          @if($canCreate)
                    ...
                    <div class="d-flex gap-2">
                      @if($canUpdate)
                        <button type="button" class="btn btn-sm btn-outline-primary js-edit" data-bs-toggle="modal"
                          ...
                          data-update-url="{{ route('admin.account_types.update', $r->id) }}"
                      ...
                      @if($canUpdate)
                        <form action="{{ route('admin.account_types.toggle', $r->id) }}" method="POST">
                          @csrf
      ...
      <div class="modal-content">
        <form method="POST" action="{{ route('admin.account_types.store') }}">
          ...
          <div class="modal-header">
            <h5 class="modal-title">{{ __('master_data.account_types.add_modal_title') }}</h5>
          ...
          <div class="modal-body">
            ...
            <input type="text" name="name" class="form-control" value="{{ old('name') }}" placeholder="{{ __('master_data.account_types.placeholders.name') }}" required>
            <small class="text-muted">{{ __('master_data.account_types.hint') }}</small>
          </div>
  ...
  @php
    $editRow = old('modal_context') === 'edit'
      ? $types->firstWhere('id', (int) old('id'))
      : null;
  @endphp
  ...
  <div class="modal fade" id="editModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        <form method="POST" action="{{ $editRow ? route('admin.account_types.update', $editRow->id) : '#' }}" id="editForm">
          @csrf
          @method('PUT')
          ...
          <div class="modal-header">
            <h5 class="modal-title">{{ __('master_data.account_types.edit_modal_title') }}</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body">
