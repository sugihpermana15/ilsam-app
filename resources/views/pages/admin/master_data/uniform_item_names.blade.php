...
@section('title', __('master_data.uniform_item_names.page_title') . ' | IGI')

@section('title-sub', __('master_data.uniform_item_names.subtitle'))
@section('pagetitle', __('master_data.uniform_item_names.pagetitle'))
...
@section('content')
  @php
    $canCreate = \App\Support\MenuAccess::can(auth()->user(), 'uniform_item_names', 'create');
    $canUpdate = \App\Support\MenuAccess::can(auth()->user(), 'uniform_item_names', 'update');
  @endphp
  <div class="row">
    ...
    <div class="col-12">
      <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
          <h5 class="card-title mb-0">{{ __('master_data.uniform_item_names.card_title') }}</h5>
                  ...
                  <td>
                    <div class="d-flex gap-2">
                      <button type="button" class="btn btn-sm btn-outline-primary js-edit" data-bs-toggle="modal"
                        ...
                        data-update-url="{{ route('admin.uniform_item_names.update', $r->id) }}"
                        data-id="{{ $r->id }}" data-name="{{ $r->name }}" data-active="{{ $r->is_active ? '1' : '0' }}">
                        <i class="fas fa-pen"></i>
                      </button>

                      <form action="{{ route('admin.uniform_item_names.toggle', $r->id) }}" method="POST">
                        @csrf
                        <button type="submit" class="btn btn-sm {{ $r->is_active ? 'btn-outline-secondary' : 'btn-outline-success' }}" title="{{ $canUpdate ? __('common.change_status') : __('common.no_access_update') }}" {{ $canUpdate ? '' : 'disabled' }}>
  ...
  <div class="modal fade" id="addModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        <form method="POST" action="{{ route('admin.uniform_item_names.store') }}">
          @csrf
          <input type="hidden" name="modal_context" value="create">
          <div class="modal-header">
            <h5 class="modal-title">{{ __('master_data.uniform_item_names.add_modal_title') }}</h5>
  ...
  @php
    $editRow = old('modal_context') === 'edit'
      ? $itemNames->firstWhere('id', (int) old('id'))
      : null;
  @endphp
  ...
  <div class="modal fade" id="editModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        <form method="POST" action="{{ $editRow ? route('admin.uniform_item_names.update', $editRow->id) : '#' }}" id="editForm">
          @csrf
          @method('PUT')
          ...
          <div class="modal-header">
            <h5 class="modal-title">{{ __('master_data.uniform_item_names.edit_modal_title') }}</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body">
